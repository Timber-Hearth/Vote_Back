...
def VoteProcess(db, poll_data: PollGroup, normalized_annonymou_id: str, options: list[int]):
    try:
        if db == None:
        ...
        datas = db.query(PollGroup).where(PollGroup.qr_token == poll_data.qr_token).first()
        if datas is None:
